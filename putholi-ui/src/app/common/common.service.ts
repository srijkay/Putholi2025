...
export class CommonService {
	...
	callApi(url, data, method, isForm?, isPublic?, islogin?) {
		...
		let headers;
		if (isPublic) {
			headers = new HttpHeaders({ 'Accept-Language': 'en', 'content-Type': 'application/json', 'loggedUser': 'admin', 'Authorization': 'Basic Y3JpbXNvbjpjUmlNczBuJGRjMW5EMUE=' });

		} else {

			headers = new HttpHeaders({ 'Accept-Language': "en-us", 'loggedUser':this.getToken('username'), 'Authorization': sessionStorage.getItem('accessToken') });
		}
		if (isForm) {
			headers = new HttpHeaders({ 'Accept-Language': "en-us", 'loggedUser': 'admin', 'Authorization': 'Basic Y3JpbXNvbjpjUmlNczBuJGRjMW5EMUE=' });
		}
		if (islogin === 'REG') {
			if (method === 'post') {
	...
	downloadApproval(id) {
		let headers = new HttpHeaders({ 'Accept-Language': "en", 'Content-Type': 'application/json', 'Authorization': sessionStorage.getItem('accessToken') });
		return this._http.get(this._apiUrl + '/v1/api/attachments/school/' + id + '/DR', {
	...
	getPdfDocument(data) {
		let corpId = this.getOrgId();
		let headers = new HttpHeaders({ 'Accept-Language': 'en-US', 'Authorization': sessionStorage.getItem('accessToken') });
		return this._http.get(this._apiUrl + '/v1/api/' + 'orgId/' + corpId + '/report-view/' + data + '/pdf', {
			responseType: 'arraybuffer' as 'json', headers: headers, observe: 'response'
		});
	}
